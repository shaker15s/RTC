    ...
    useEffect(() => {
        ...
        const runScribbleAnimation = (e) => {
            ...
            if (!transitionLogo) {
                ...
                const mainLogo = document.querySelector('.logo-rtc');
                if (mainLogo) {
                    const svgClone = mainLogo.cloneNode(true);
            ...
            drawTl.to(transitionLogo, {
                ...
                onStart: () => {
                    const cloneSvg = transitionLogo.querySelector('svg');
                    if (cloneSvg) {
                        gsap.to(cloneSvg, { rotation: 5, duration: 0.15, repeat: -1, yoyo: true, ease: 'steps(1)', overwrite: 'auto' });
                    }
                }
            }, durIn * 0.5);
            ...
            drawTl.set(transitionLogo, {
                ...
                onComplete: () => {
                    const cloneSvg = transitionLogo.querySelector('svg');
                    if (cloneSvg) {
                        gsap.killTweensOf(cloneSvg);
                        gsap.set(cloneSvg, { rotation: 0 });
                    }
                }
            }, durIn + (durOut * 0.48));
        };
        ...
        const mainLogo = document.querySelector('.logo-rtc');
        if (mainLogo) {
            mainLogo.addEventListener('click', runScribbleAnimation);
        }

        // Auto-trigger scribble transition on page load
        const entryTimeout = setTimeout(() => {
            runScribbleAnimation();
        }, 300);

        return () => {
            const activeLogo = document.querySelector('.logo-rtc');
            if (activeLogo) {
                activeLogo.removeEventListener('click', runScribbleAnimation);
            }
            clearTimeout(entryTimeout);
            const leftover = document.querySelector('.transition-logo');
